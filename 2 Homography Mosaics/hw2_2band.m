...
% 7. construct low and high frequency band for rgb and alpha
...
% 8. combine the results using alpha blending
J_L = zeros(size(A_L{1}));
J_H = zeros(size(A_H{1}));
for i = 1:nimages
    J_L = J_L + A_L{i} .* I_L{i};  % low frequency band
    J_H = J_H + A_H{i} .* I_H{i};   % high frequency band
end
K = J_L + J_H; % combined bands
